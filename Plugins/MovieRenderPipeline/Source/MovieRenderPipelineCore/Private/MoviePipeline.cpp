...
void UMoviePipeline::Initialize(UMoviePipelineExecutorJob* InJob)
{
	...
	const bool bIsEmptyPlaybackRange = TargetSequence->GetMovieScene()->GetPlaybackRange().IsEmpty();
	if (bIsEmptyPlaybackRange)
	{
		UE_LOG(LogMovieRenderPipeline, Error, TEXT("Playback Range was zero. End Frames are exclusive, did you mean [n, n+1]?"));
	}

	// Warn if the shot mask entirely disabled everything.
	const bool bAllShotsDisabled = ActiveShotList.IsEmpty();
	if (bAllShotsDisabled)
	{
	...
	// If there's no work to do, shut down.
	if (bIsEmptyPlaybackRange || bAllShotsDisabled)
	{
