...
void UMoviePipelineVideoOutputBase::OnReceiveImageDataImpl(FMoviePipelineMergerOutputFrame* InMergedOutputFrame)
{
	...
	for (TPair<FMoviePipelinePassIdentifier, TUniquePtr<FImagePixelData>>& RenderPassData : InMergedOutputFrame->ImageOutputData)
	{
		...
		if(!OutputWriter)
		{
			// If overscan is going to be cropped, use the output resolution from the crop rectangle. Otherwise, use the internal resolution from the
			// raw render pass data
			FIntPoint OutputResolution = RenderPassData.Value->GetSize();
			if (ShouldCropOverscanImpl())
			{
				OutputResolution = Payload->SampleState.CropRectangle.Size();
			}
			...
			TUniquePtr<MovieRenderPipeline::IVideoCodecWriter> NewWriter = Initialize_GameThread(FinalFilePath,
				OutputResolution, RenderPassData.Value->GetType(), RenderPassData.Value->GetPixelLayout(),
		...
		if (ShouldCropOverscanImpl() && bIsCropRectValid && bCanCropResolution)
		{
			...
			case EImagePixelType::Color:
				{
					TAsyncCropImage<FColor> CropImagePreprocessor(Payload->SampleState.CropRectangle);
			...
			case EImagePixelType::Float16:
				{
					TAsyncCropImage<FFloat16Color> CropImagePreprocessor(Payload->SampleState.CropRectangle);
					CropImagePreprocessor(RawRenderPassData);
					CroppedPixelData = MoveTemp(CropImagePreprocessor.OutCroppedImage);
				}
				break;
			...
			case EImagePixelType::Float32:
				{
					TAsyncCropImage<FLinearColor> CropImagePreprocessor(Payload->SampleState.CropRectangle);
					CropImagePreprocessor(RawRenderPassData);
					CroppedPixelData = MoveTemp(CropImagePreprocessor.OutCroppedImage);
				}
				break;
		...
		MovieRenderPipeline::IVideoCodecWriter::FLightweightSourceData& LightweightData = OutputWriter->Get<0>()->LightweightSourceData.FindOrAdd(Payload->SampleState.OutputState.ShotIndex);
